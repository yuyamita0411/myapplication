...
<script type="module">
    ...
    window.addEventListener('load', (e) => {
        ...
        commonloadMotion('ScheduleLoadingArea', "{{ secure_asset('asset/loadingicon.png') }}");
    ...
    window.addEventListener('click', (e) => {
        ...
        if(e.target.className.match(/calenderBtn/)){
            ...
            <div class="col-12 d-flex">
                <img
                src="{{ secure_asset('asset/timeicon.png') }}"
            ...
            document.getElementById("commonmodal").action = "{{ route('schedule.schedule.add') }}";
        ...
        if(e.target.className.match(/rebaseschedule/)){
            ...
            document.getElementById("commonmodal").action = "{{ route('schedule.schedule.rebase') }}";
            MakeEditSchedule(e);
        }
    ...
    window.addEventListener('input', (el) => {
        if (el.target.id.match(/addgroupmember/)) {
            ajax.method1(
                "{{route('schedule.schedule.search')}}",
                'POST',
                {
    ...
    const MakeEditSchedule = (e) => {
        ...
        ajax.method1(
            "{{route('schedule.schedule.fromid')}}",
            'POST',
            {
            ...
            (Res) => {
                ...
                <div class="col-12 d-flex">
                    <img src="{{ secure_asset('asset/timeicon.png') }}" class="searchicon cursor p-1 mr-3 tooltip-top" data-tooltip="時刻を設定する">
                    <div class="w-100 text-center pr-2 position-relative">
